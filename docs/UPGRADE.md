...
5. **Merges v6 settings** into your existing `.claude/settings.json`:
   ...
   - **v6.2.0**: adds the **read-only quality gates** `permissions.deny` block (makes `.quality-gates.json` and `gates/` unwritable by agents; the rules use the `Edit(path)` form, which Claude Code applies to every file-editing tool (Edit, Write, MultiEdit, NotebookEdit), whereas `Write()`/`MultiEdit()` rule forms are silently ignored, see A11-ISS-7) and a blocking **PreToolUse "read-only gate guard"** hook (refuses Bash write forms against gate paths through 13 detection branches as of v6.3.0 — redirection, `tee`, `sed -i`, `cp`, `mv`, `rm`/`truncate`/`shred`/`unlink`, `dd of=`, `ln -s`, `perl`/`ruby -i`, interpreter one-liners naming a gate path literally beside a write verb, gate paths held in a shell variable, `patch`/`git apply`, `git checkout`/`restore`/`rm`/`mv`, and a write verb fed through `xargs`. It still does **not** catch a path assembled at runtime, an interpreter reaching the path indirectly, anything eval'd or base64-decoded, or a write done by a program the command launches — no shell hook can. Treat it as a speed bump rather than a closed route, see A11-ISS-16; the guard logic lives in `.claude/hooks/gate-guard.sh`, deployed alongside)
...
### v6.3.0: what an upgrade adds on top of v6.2.0

- **`## ORIENTATION PROTOCOL`** in all 11 specialists and all 18 missions. No configuration; it arrives with the re-deployed library.
- **Five missions that previously could not be invoked** — `architecture`, `product-description`, `operation-genesis`, `connect-mcp`, `operation-recon`. They installed correctly and then hard-errored on `/coord`, because the routing table listed 13 of 18 (A11-ISS-17). `connect-mcp` and `operation-recon` were additionally missing from the installer's mission list, so no project ever received the files at all.
- **`.claude/scripts/mission-state.py`** and `.claude/state/mission-state.json` — the coordinator's phase counters, error budget and clean-round tally, on disk rather than narrated.
- **A wider Bash gate guard** (5 inline command forms → 13 branches). Regression-tested so ordinary Bash is unaffected: `npm test`, `git status gates/`, `rm -rf node_modules`, `python3 gates/run-gates.py`, reading a gate through an interpreter, and assigning a gate path to a variable that is never written through all pass.
- **`.claude/agent-11-version`**, a version stamp written on every install. An install from a tree with entries under `[Unreleased]` is stamped `X.Y.Z+unreleased`.
- **Backups now land in `~/.agent-11/backups/`**, outside the repo. Override with `AGENT11_INSTALL_BACKUPS`. Existing in-repo backups are left alone; they are safe to delete once you have checked them.
- **MCP setup is opt-in.** Pass `--with-mcp` if you want it. Earlier versions auto-ran `mcp-setup.sh` whenever `.env.mcp` was present, which meant global npm installs and `claude mcp remove -s project` for 10 servers as a side effect of an unrelated upgrade (A11-ISS-23).

Nothing in this release changes the `permissions.deny` set — it is byte-identical to v6.2.0's. Documentation that overclaimed what those rules cover was corrected in eleven places.
